...
# @numba.jit
def take_derivative(u, grid, dif_type):
    const = dif_type[0]
    var = dif_type[1]
    order = dif_type[2]
    power = dif_type[3]
    ...
    for i in range(order):
        ...
        der = derivative(der, h, var)
    der = const * der ** power
    der = np.array(der, dtype=np.float64)
    return der


def apply_const_operator(u, grid, operator):
    total = 0
    for term in operator:
        up = u
        for dif in term:
            up = take_derivative(up, grid, dif)
        total += up
    return total
...
def nn_interp(matrix, grid, new_grid):
    X, T = np.meshgrid(grid[1], grid[0])
